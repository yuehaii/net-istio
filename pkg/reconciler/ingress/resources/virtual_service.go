...
func makeVirtualServiceRoute(hosts sets.Set[string], http *v1alpha1.HTTPIngressPath, gateways map[v1alpha1.IngressVisibility]sets.Set[string], visibility v1alpha1.IngressVisibility) *istiov1beta1.HTTPRoute {
	...
	for _, split := range http.Splits {
		...
		// retrieve the service host domain from istio cluster domain setting,
		// the vs destination host should match the endpoints generated by EDS
		serviceHostName := IstioClusterDomain
		if len(serviceHostName) == 0 {
			// if can't get the istio cluster domain, try to parse it from resolve.conf
			serviceHostName = network.GetServiceHostname(split.ServiceName,
				split.ServiceNamespace)
		}
